...
void utilities::parameter_information( grid::parameters grid ) {
  ...
  while( iter < 2 ) {
    ...
    cout << fixed      << setprecision(3)  << "Cosmological constant = " << COSMOLOGICAL_CONSTANT      << endl;
...
void utilities::compute_and_output_mass_aspect_function( const int which_level, const int n, const grid::parameters grid, const gridfunction a ) {
  ...
  LOOP(0,Nx0Total) {
    ...
    const real mass    = 0.5 * r_ito_x0[j] * ( 1.0 - 1.0/SQR(a_local)- SQR(r_ito_x0[j])/l2 );
...
void utilities::output_energy_density_to_file( const grid::parameters grid, const realvec Phi, const realvec Pi, const realvec a, const int n ) {
  ...
  LOOP(0,Nx0Total) {
    ...
    const real rho      = 0.5 * EPSILON * ( Phi_sqrd + Pi_sqrd ) / a_sqrd;
    /* Then output to file */
